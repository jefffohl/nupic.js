...
Encoder.prototype = {
   ...
   * If no arguments are supplied, returns the names of the fields.
    ...
    if (!util.isNullOrUndefined(this.getEncoders())) {
      ...
      for (var tuple in encoders) {
        ...
        if (!util.isNullOrUndefined(parentFieldName)) {
          for (var name1 in subNames) {
            hierarchicalNames.push(parentFieldName + "." + name1);
          }
        }
        for (var name2 in hierarchicalNames) {
          names.push(name2);
        }
      }
    } else {
  ...
  pprint: function(output, prefix) { // void(int[], String)
    ...
    for (var i = 0; i < len; i++) {
      ...
      this.LOGGER.info(ArrayUtils.bitsToString(ArrayUtils.sub(output, ArrayUtils.range(offset, nextOffset))));
    }
  },
